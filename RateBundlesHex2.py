...
def grid( theta, aRA, aDEC, epsilon=0.04, full=False, count_thresh=3, deltav=[] ):
    ...
    for i in range(0, len(X)):
        ...
        if ok >= count_thresh:
            ...
            if deltav != []:
                dv.append( 'Total pop frac: %.3f Targetable frac: %.3f'%( numpy.mean( r2 <= epsilon**2 ), numpy.mean( deltav[ r <= epsilon**2 ] ) ) )
            else:
                dv.append( 0 )
    ...
    if full:
        dv = numpy.asarray( dv ) [ numpy.argsort( inds ) ]
        return count, Xk, Yk, numpy.sort( inds ), dv
    else:
        return count  
...
def extract_offsets(astrom_line):
        ...
        for i in range(0, len(astrom_line)):
            ...
            all_RA.append( RA )
            all_DEC.append( DEC )
            #RA_GEO.append(  float(astrom_line[i][-2]) )
            #DEC_GEO.append( float(astrom_line[i][-1]) )
        ...
        return numpy.asarray( shiftRA ), numpy.asarray( shiftDEC ), numpy.asarray( all_RA ), numpy.asarray( all_DEC ), cross_deltaRA, cross_deltaDEC #, RA_GEO, DEC_GEO
...
def opt_rates( astrom, datev, orbv ):
    ...
    print 'MEAN ENCOUNTER PROB: '%( numpy.mean( deltav < 0.130 ) )
    ...
    N, Xk, Yk, keep, dv_frac = grid( best_theta, sRA, sDEC,epsilon=scale_term*0.04, full=True, deltav=deltav )

    print N, len(keep), keep

    for i in range(0, len(keep)):
        print i, keep[i], dv_frac[i], orbv[keep[i]].strip()

    ax.scatter(sRA, sDEC, c='k', zorder=-1)
    ax.scatter(sRA[keep], sDEC[keep], c='cyan', zorder=2 )
    ...
    for i in keep:
        ...
        #ax.annotate( xy=( sRA[i], sDEC[i] ), s=str(c).rjust(2, '0'), color='r' )
        c += 1
...
def get_ephem( ):
    ...
    FILE = open('hubble_orbit_ephemeris_pilot_jd.asc', 'r')
    dat  = FILE.readlines()
    FILE.close()
...
def get_flt(root_path):
    fn = commands.getoutput('ls %s/*_flt.fits'%(root_path)).split('\n')
    dx = []
    km2AU = 1.0 / 1.49598e8
    ...
    for k in fn:
        ...
        dx, dy, dz = Xi(date_midexp), Yi(date_midexp), Zi(date_midexp)
        
        dx.append( [ date_midexp, dx, dy, dz, k.split('_')[0] ] )
...
if __name__ == '__main__':
    ...
    for d in dirlist:
        ...
        #if os.path.isfile('/Users/spica/NHA/HST/demo_shifts/%s_shifts.tar'%(  datev[0][-1][0:6] ) ):
        #    print 'Seems like tarball already exists; will not regenerate.'
        #    os.chdir('../')
        #    continue
        #else:
