...
def do_main(datev):
    FILE = open('/Users/spica/scripts/git_projects/skyrate/NH_ENC_equal.txt', 'r')
    dat = FILE.readlines()
    FILE.close()
...
def opt_rates( astrom, datev, orbv, do_refined ):
    ...
    #    os.system('scp *.tar aparker@spikard.boulder.swri.edu:/net/suhuy/raid1/buie/hst/13633/shifts/')
    #else:
    #    os.system('cp *.tar /Users/spica/NHA/HST/final_shifts/')
    #    #os.system('cp *.tar /Users/spica/NHA/HST/demo_shifts/hstshifts/')
...
if __name__ == '__main__':
    ...
    for d in dirlist:
        ...
        if os.path.isfile('%s_shifts.tar'%(  datev[0][-1][0:6] ) ) and not do_refined:
            print 'Seems like tarball already exists; will not regenerate.'
            os.chdir('../')
            continue
        else:
        ...
        best_orb_ind = opt_rates( astrom, datev, orbv, do_refined )
        os.chdir('../')
